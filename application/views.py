...
from application.models import Likes, Posts
from application.permissions import IsOwnerOrAdmin
from application.scripts import delete_object
from application.serializers import (CommentsSerializer, PostSerializer,
                                     UserSerializer)
    ...
    def like(self, request, pk=None):
        ...
        if request.method == "DELETE":
            return delete_object(
                ...
                errors_message="Уже лайкнуто",
            )
        if if_already_exists:
            return Response(
                {"errors": "Уже лайкнуто"}, status=status.HTTP_400_BAD_REQUEST
            )
        else:
            like = Likes(**fields)
            like.save()
